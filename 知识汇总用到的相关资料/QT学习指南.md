...
qDebug() << "Hello World";        
// 支持使用占位符
// %1 替换为 name，%2 替换为 age
qDebug() << QString("名称：%1，年龄：%2").arg(name).arg(age);

// 更多输出方法建议直接问AI
```

##### 2\. QByteArray 和 QString

| 特性             | `QByteArray`                                | `QString`                               |
| ---------------- | ------------------------------------------- | --------------------------------------- |
| **存储内容**     | 原始字节数据（二进制数据或 ASCII 文本）     | Unicode 字符串（支持多语言文本）        |
| **内部编码**     | 无特定编码，直接接存储字节（`char` 数组）   | UTF-16（Qt 内部统一编码，支持全球语言） |
| **空值表示**     | 允许包含 `\0`（字节 0），长度不受 `\0` 影响 | 字符串中可包含 `\0`，但会被视为正常字符 |
| **典型数据示例** | 图片二进制流、网络数据包、文件字节流        | 界面文本、用户输入、多语言字符串        |
