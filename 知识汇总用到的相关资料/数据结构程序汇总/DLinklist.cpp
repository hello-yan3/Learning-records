 ...
 * @attention：
 * 双链表的基本概念
 * 1. 双链表：双链表也是链表的一种。双链表的每个数据节点中都有两个指针，分别指向前驱节点和后继结点。
 * 2. 特点：
 * * 优点：可正向/逆向检索
 * * 缺点：存储密度更低
 */
...
DLinklist List_HeadInsert(DLinklist &L)
{
    ...
    {
        // 初始：  [NULL]L[NULL]
        // 初始：  [NULL]L[s1]  [L]s1(x1)[NULL]   ……
        // 初始：  [NULL]L[s2]  [s2]s1(x1)[NULL]   [L]s2(x2)[s1]
        // 整理：  [NULL]L[s2]  [L]s2(x2)[s1]  [s2]s1(x1)[NULL]  ……
        // 插入过程中保存头结点不变；
...
bool InsertDNodeNextDNode(DNode *p, DNode *s)
{
    ...
    s->next = p->next;
    // 判断结点p之后是否有后继结点
    if (p->next != NULL) // 对于最后一个结点，next结点为null，p->next->prior=s会产生的空指针问题
        p->next->prior = s;
    s->prior = p;
    p->next = s;
    return true;
...
1 2 3 4 5 6 7 9999
地址00000146625F5EE0内存放数据为1,下一个地址为00000146625F5F30
地址00000146625F5F30内存放数据为2,下一个地址为00000146625F5F80
地址00000146625F5F80内存放数据为3,下一个地址为00000146625FC4A0
地址00000146625FC4A0内存放数据为4,下一个地址为00000146625FC4F0
地址00000146625FC4F0内存放数据为5,下一个地址为00000146625FC540
地址00000146625FC540内存放数据为6,下一个地址为00000146625FC590
地址00000146625FC590内存放数据为7,下一个地址为0000000000000000
链表长度为：7

链表为空? 0 | (0)假 (1)真

地址00000146625F5EE0内存放数据为1,下一个地址为00000146625F5F30
地址00000146625F5F30内存放数据为2,下一个地址为00000146625FD610
地址00000146625FD610内存放数据为100,下一个地址为00000146625F5F80
地址00000146625F5F80内存放数据为3,下一个地址为00000146625FC4A0
地址00000146625FC4A0内存放数据为4,下一个地址为00000146625FC4F0
地址00000146625FC4F0内存放数据为5,下一个地址为00000146625FC540
地址00000146625FC540内存放数据为6,下一个地址为00000146625FC590
...
该位置处 节点地址为:00000146625FD610, 其值为100

地址00000146625F5EE0内存放数据为1,下一个地址为00000146625F5F30
地址00000146625F5F30内存放数据为2,下一个地址为00000146625FD610
地址00000146625FD610内存放数据为100,下一个地址为00000146625FD660
地址00000146625FD660内存放数据为20,下一个地址为00000146625F5F80
地址00000146625F5F80内存放数据为3,下一个地址为00000146625FC4A0
地址00000146625FC4A0内存放数据为4,下一个地址为00000146625FC4F0
地址00000146625FC4F0内存放数据为5,下一个地址为00000146625FC540
地址00000146625FC540内存放数据为6,下一个地址为00000146625FC590
...
该位置处 节点地址为:00000146625FC4F0, 其值为5
...
地址00000146625FD660内存放数据为20,下一个地址为00000146625F5F80
地址00000146625F5F80内存放数据为3,下一个地址为00000146625FC4A0
地址00000146625FC4A0内存放数据为4,下一个地址为00000146625FD6B0
地址00000146625FD6B0内存放数据为25,下一个地址为00000146625FC4F0
地址00000146625FC4F0内存放数据为5,下一个地址为00000146625FC540
地址00000146625FC540内存放数据为6,下一个地址为00000146625FC590
地址00000146625FC590内存放数据为7,下一个地址为0000000000000000
链表长度为：10
...
地址00000146625F5EE0内存放数据为1,下一个地址为00000146625FD610
地址00000146625FD610内存放数据为100,下一个地址为00000146625FD660
地址00000146625FD660内存放数据为20,下一个地址为00000146625F5F80
地址00000146625F5F80内存放数据为3,下一个地址为00000146625FC4A0
地址00000146625FC4A0内存放数据为4,下一个地址为00000146625FD6B0
...
地址00000146625F5F80内存放数据为3,下一个地址为00000146625FC4A0
地址00000146625FC4A0内存放数据为4,下一个地址为00000146625FD6B0
地址00000146625FD6B0内存放数据为25,下一个地址为00000146625FC540
地址00000146625FC540内存放数据为6,下一个地址为00000146625FC590
地址00000146625FC590内存放数据为7,下一个地址为0000000000000000

逆向遍历链表
地址00000146625FC590内存放数据为7,上一个地址为00000146625FC540
地址00000146625FC540内存放数据为6,上一个地址为00000146625FD6B0
地址00000146625FD6B0内存放数据为25,上一个地址为00000146625FC4A0
地址00000146625FC4A0内存放数据为4,上一个地址为00000146625F5F80
地址00000146625F5F80内存放数据为3,上一个地址为00000146625FD660
地址00000146625FD660内存放数据为20,上一个地址为00000146625FD610
地址00000146625FD610内存放数据为100,上一个地址为00000146625F5EE0
地址00000146625F5EE0内存放数据为1,上一个地址为00000146625F5CA0
链表长度为：8
