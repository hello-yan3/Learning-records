...
| 宏           | 描述                                                         |
| :----------- | :----------------------------------------------------------- |
| \_\_DATE\_\_ | 当前日期，一个以 "month/day/year " 格式表示的字符常量。如：Feb 28 2011 |
| \_\_TIME\_\_ | 当前时间，一个以 "hour:minute:second" 格式表示的字符常量。如：18:52:48 |
| \_\_FILE\_\_ | 这会包含当前文件名，一个字符串常量。如：test.cpp             |
| \_\_LINE\_\_ | 这会包含当前行号，一个十进制常量。如：15                     |
...
| signed short int   | 1 (8位) 或 2 (16位) 字节  | -128 到 127 或 -32768 到 32767                               |
...
| **volatile**         | 修饰符 **volatile** 告诉编译器不需要优化volatile声明的变量，让程序可以直接从内存中读取变量。<br />对于一般的变量编译器会对变量进行优化，将内存中的变量值放在寄存器中以加快读写效率。<br />常用与**频繁使用的变量**或**多线程使用变量**<br />更多方法见**[C/C++ 中 volatile 关键字详解 ](https://www.runoob.com/w3cnote/c-volatile-keyword.html)** |
...
修饰**局部变量**——可以在函数调用之间保持局部变量的值（或者说此变量性质类似全局变量，但是作用域仅局限于函数内）；
...
![](https://static.runoob.com/wp-content/uploads/c/c-if-else-20200923.svg)
...
| **strcmp(s1, s2);** | string compare ，如果 s1 和 s2 是相同的，则返回 0；<br />如果两个字符串不相同，且第一个字符串的第一个字符小于第二个字符串的第一个字符，返回值＜ 0，反之，返回值＞0。 |
...
**二维数组访问见：[评论笔记3、4](https://www.runoob.com/cprogramming/c-multi-dimensional-arrays.html)**
...
#### 17.3 [C 指针数组](https://www.runoob.com/cprogramming/c-array-of-pointers.html)
...
等效于：int \*(ptr[3]);
**数组指针：**表示的是这是个**指向多维数组的指针**，那么该指针变量存储的地址就必须是数组的首地址
数组指针定义举例：int (\*p)[3];（()优先级最高，先与\*结合，所以是指针，之后与[]结合，说明指针指向数组）
数组指针的使用：int pp\[2][2]={{1,2},  {3,4}};     int (\*p)[2] = pp;
...
Pt2 = &Pt1;  // Pt2 获取 Pt1 的地址   // Pt2 = 0x7ffee2d5e8d0，**Pt2 = 100
```
...
// 指针指向的是同一类型的不同结构体，这里存储的是 地址 信息，所以不会无线嵌套；
