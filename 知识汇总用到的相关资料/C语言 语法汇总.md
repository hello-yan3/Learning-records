...
| 宏名称                    | 编译时替换内容                                               |
| ------------------------- | ------------------------------------------------------------ |
| **`__func__`**            | 当前函数的名称（字符串常量，如 `"main"`、`"calculateSum"`）  |
| **`__FUNCTION__`**        | 同 `__func__`（部分编译器扩展，行为一致）                    |
| **`__FILE__`**            | 当前源文件的路径（字符串常量，如 `"D:/project/main.cpp"`）   |
| **`__LINE__`**            | 当前代码行的行号（整数常量，如 `42`）                        |
| **`__DATE__`**            | 编译时的日期（字符串常量，格式：`"Mmm dd yyyy"`，如 `"Oct 05 2024"`） |
| **`__TIME__`**            | 编译时的时间（字符串常量，格式：`"hh:mm:ss"`，如 `"14:30:59"`） |
| **`__STDC__`**            | 若编译器遵循 C 标准，则定义为 `1`（C++ 中通常不定义，或定义为其他值） |
| **`__cplusplus`**         | 整数常量，代表 C++ 标准版本（如 C++11 为 `201103L`，C++17 为 `201703L`） |
| **`__INT_MAX__`**         | `int` 类型的最大值（整数常量，如 32 位系统为 `2147483647`）  |
| **`__INT_MIN__`**         | `int` 类型的最小值（整数常量，如 32 位系统为 `-2147483648`） |
| **`__LONG_MAX__`**        | `long` 类型的最大值（整数常量，如 64 位系统为 `9223372036854775807`） |
| **`__SIZE_MAX__`**        | `size_t` 类型的最大值（无符号整数常量，如 64 位系统为 `18446744073709551615`） |
| **`__VA_ARGS__`**         | 可变参数宏中，代表所有可变参数（需配合 `#define` 定义可变参数宏） |
| **`__PRETTY_FUNCTION__`** | 当前函数的 “美化名称”（含函数签名，如 `"int calculateSum(int, int)"`） |
